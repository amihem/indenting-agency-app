...
import { indentOrderValue, totalDispatchedQty, pendingQty, computeDispatchFinancials, roundRupee } from "../lib/calc";
...
function DispatchSection({ indent, addDispatch, updateDispatch, deleteDispatch }) {
  ...
  const preview = computeDispatchFinancials(previewTaxableValue, form.freight);
  ...
  return (
    <div style={{ borderTop: `1px dashed ${colors.border}`, marginTop: 10, paddingTop: 10 }}>
        ...
        <div style={{ ...styles.card, padding: 12, marginBottom: 10 }}>
          ...
          <div style={{ ...styles.card, padding: 10, marginBottom: 12, background: colors.bg }}>
            <div style={{ display: "flex", justifyContent: "space-between", fontSize: 12, marginBottom: 4 }}>
              <span>GST @ 5% (on Value + Freight)</span>
              <strong>{formatINR(preview.gst)}</strong>
            </div>
            <div style={{ display: "flex", justifyContent: "space-between", fontSize: 12, marginBottom: 4 }}>
              <span>Round Off (auto)</span>
              <strong>{preview.roundOff >= 0 ? "+" : ""}{formatINR(preview.roundOff)}</strong>
            </div>
            <div style={{ display: "flex", justifyContent: "space-between", fontSize: 13, borderTop: `1px solid ${colors.border}`, paddingTop: 6, marginTop: 4 }}>
              ...
              <strong style={{ color: colors.primary }}>{formatINR(preview.grandTotal)}</strong>
            </div>
          </div>
      ...
      {dispatches.map((disp) => {
        const dTaxable = roundRupee((Number(disp.qty) || 0) * (Number(indent.rate) || 0));
        const dFin = computeDispatchFinancials(dTaxable, disp.freight);
        return (
          <div key={disp.id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", fontSize: 12, padding: "6px 0", borderBottom: `1px solid ${colors.border}`, gap: 6, flexWrap: "wrap" }}>
            <div>
              ...
              <div style={{ color: colors.textMuted, marginTop: 2 }}>
                Value: {formatINR(dTaxable)} · Freight: {formatINR(dFin.freight)} · GST 5%: {formatINR(dFin.gst)} · R/off: {dFin.roundOff >= 0 ? "+" : ""}{formatINR(dFin.roundOff)} · Grand Total: <strong>{formatINR(dFin.grandTotal)}</strong>
              </div>
            </div>
