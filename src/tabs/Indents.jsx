...
function DispatchSection({ indent, addDispatch, updateDispatch, deleteDispatch }) {
  ...
  // Live preview of Freight / GST(5%) / Round Off / Invoice Total as the user types
  const previewQty = Number(form.qty) || 0;
  const previewUnitValue = Math.round(previewQty * (Number(indent.rate) || 0));
      ...
      {showForm && (
        <div style={{ ...styles.card, padding: 12, marginBottom: 10 }}>
          ...
          <div style={styles.row2}>
            ...
            <div>
              <label style={styles.label}>GST @ 5% (on Unit Value + Freight)</label>
              <input style={{ ...styles.input, background: colors.bg }} value={formatINR(previewGst)} readOnly />
            </div>
          </div>
      ...
      {dispatches.map((disp) => (
        <div key={disp.id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", fontSize: 12, padding: "6px 0", borderBottom: `1px solid ${colors.border}`, gap: 6 }}>
          <div>
            ...
            {Number(disp.freight) > 0 ? ` · Freight: ${formatINR(disp.freight)}` : ""}
          </div>
